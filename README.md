...
エフェクト（Plugins）のフォルダは写真現像のプロセスに基づいて管理します。
...
Coreシステムは起動時に `src/plugins` 以下のすべてのプラグインを読み込み、レイヤーで指定された順序に基づいて適用します。
...
1.  **新規エフェクト作成**:
      * `src/plugins` 内の適切なカテゴリのフォルダに新規フォルダを作成。
